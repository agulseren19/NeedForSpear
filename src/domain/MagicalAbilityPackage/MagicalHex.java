...
public class MagicalHex extends MagicalAbility {
	...
	public void checkHitAndUpdateLocation() {
		...
		// MODIFIES: rectOfHex, (when there is no hit on obstacle) xLocation and
		// yLocation
		// EFFECTS: Checks whether the hex hit any obstacle or not in that timer period.
		// If the hex doesn't hit, then updates location. If it hits any, then removes
		// breaks that obstacle.
		if (this.getPaintTime() == GameStartup.getTimerCount()) {
			...
			// As there are so much obstacles to hit, and it is sometimes hard to continue
			// playing.
			// We wanted magical hex to hit more than one obstacle to help player approach
			// to winning.
