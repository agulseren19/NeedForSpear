...
public class RewardBox implements Serializable {
	...
	public void checkCatchAndUpdateLocation() {
		...
		if (this.getPaintTime() == GameStartup.getTimerCount()) {
			...
			if ((yLocation < Phantasm.getInstance().getY() + Phantasm.getInstance().getThickness())
					&& (yLocation > Phantasm.getInstance().getY()) && (xLocation > Phantasm.getInstance().getX())
					&& (xLocation < Phantasm.getInstance().getX() + Phantasm.getInstance().getLength())
					&& (Phantasm.getInstance().getRotationAngle() == 0)) {
				...
				if (isCaught == 1) {
					...
					&& (((((Phantasm.getInstance().getX() + (Phantasm.getInstance().getLen() / 2))
							...
							((((Phantasm.getInstance().getX() + (Phantasm.getInstance().getLen() / 2))
									+ ((Phantasm.getInstance().getLen() / 2) * Math
											.cos(Math.toRadians(Phantasm.getRotationAngle() * (-1))))) >= xLocation)
									&& ((Phantasm.getInstance().getX()
											+ (Phantasm.getInstance().getLen() / 2)) <= xLocation)
									&& (((Phantasm.getInstance().getY()) - ((xLocation
											- (Phantasm.getInstance().getX() + (Phantasm.getInstance().getLen() / 2)))
											* Math.tan(
													Math.toRadians(Phantasm.getRotationAngle() * (-1))))) <= yLocation
															+ sizeY))))
