...
public class SaveLoadHandler implements Serializable {
	...
	public void saveButtonClicked(String playerName) throws IOException, ClassNotFoundException {
		...
		SavedGame savedGame = new SavedGame(Player.getInstance().getScore(),
				...
				Player.getInstance().getViolentAbilityCancelTime(), ObstacleCreator.getInstance().getIndexArray(),
				EnchantedSphere.getInstance().getPauseDx(), EnchantedSphere.getInstance().getPauseDy(),
				Phantasm.getInstance().getMagicalHexCancelTime(), Phantasm.getInstance().getNextHexReleaseTime(),
